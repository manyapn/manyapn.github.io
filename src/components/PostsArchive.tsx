...
export default function PostsArchive() {
  ...
  const showFilter = posts.length > 1;
  ...
  return (
    <>
      ...
      {/* Filter bar — only when filtering would change results */}
      {showFilter && (
        <div
        ...
        >
          {years.map((year) => (
            <section key={year} style={{ marginBottom: "2.5rem" }}>
              {/* Year header — only when spanning multiple years */}
              {showYears && (
                <h2
                  style={{
              ...
              {byYear[year].map((post) => (
                <div key={post.slug} className="post-entry" style={{ borderTop: "1px solid var(--line)" }}>
